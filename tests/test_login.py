...
class LoginTest(unittest.TestCase):
	...
	def test_login_01(self):
		...
		self.log.infoLog('正确登录结果：%s' % r.json())
		self.isContent(r,1)
	...
	def test_login_02(self):
		...
		self.log.infoLog('错误密码登录结果：%s' % r.json())
		self.isContent(r,2)
	...
	def test_login_03(self):
		...
		self.log.infoLog('错误账号登录结果：%s' % r.json())
		self.isContent(r,3)
	...
	def test_login_04(self):
		...
		self.log.infoLog('错误账号密码登录结果：%s' % r.json())
		self.isContent(r,4)
	...
	def test_login_05(self):
		...
		self.log.infoLog('账号密码为空登录结果：%s' % r.json())
		self.isContent(r,5)
